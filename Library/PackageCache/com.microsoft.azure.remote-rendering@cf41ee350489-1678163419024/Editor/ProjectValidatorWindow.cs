...
namespace Microsoft.Azure.RemoteRendering.Unity.Editor
{
    public class ProjectValidatorWindow : EditorWindow
    {
        ...
        private static async void UpdateWMRPackage()
        {
            ...
            string latestVersion = package.versions.compatible.Where(x => (x.IndexOf("exp", StringComparison.OrdinalIgnoreCase) == -1 && x.IndexOf("pre", StringComparison.OrdinalIgnoreCase) == -1)).LastOrDefault();
        ...
        private void OnGUI()
        {
            ...
            if (_showRequired)
            {
                ...
                if (windowsMrEnabled)
                {
                    ...
                    {
                        AddCheckLabel("Windows XR Plugin 4.6.4 required when using UPR", !brokenConfig, currentScopeCheckLevel,
